<template>
    ...
    <q-card-section class="row justify-between">
      ...
      >
        ...
        <div class="flex row no-wrap items-center">
          ...
          <span class="text-h6 q-mr-xs">{{reservation.luggages.length}}</span>
        </div>
        <div>
  ...
  <q-card v-else>
    ...
    <q-card-section class="row justify-center row">
      ...
      <q-btn
        v-if="this.$router.currentRoute.path =='/reservation'"
        color="primary col-12 q-py-md"
        v-close-popup
      >Reserva ahora</q-btn>
      ...
      .then(response => {
        response.data.forEach(invoice => {
          ...
          invoice.start_date = moment(
            invoice.start_date.substring(0, 10)
          ).format("dddd DD/MM/YYYY");
          invoice.end_date = moment(invoice.end_date.substring(0, 10)).format(
            "dddd DD/MM/YYYY"
          );
